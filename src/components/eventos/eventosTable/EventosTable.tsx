...
export const EventosTable: React.FC<EventosTableProps> = ({
  ...
  const [estadosSeleccionados, setEstadosSeleccionados] = useLocalStorage<string[]>(ESTADOS_EVENTOS_KEY, []);
  const [vendedorSeleccionado, setVendedorSeleccionado] = useLocalStorage<string>(VENDEDOR_EVENTOS_KEY, "");
  const [fechaDesde, setFechaDesde] = useLocalStorage<string>(FECHA_DESDE_EVENTOS_KEY, "");
  const [fechaHasta, setFechaHasta] = useLocalStorage<string>(FECHA_HASTA_EVENTOS_KEY, "");
  const [searchTerm, setSearchTerm] = useLocalStorage<string>(SEARCH_TERM_EVENTOS_KEY, "");
  const [sortConfig, setSortConfig] = useLocalStorage<{ key: string; direction: 'ascending' | 'descending' | null }>(SORT_CONFIG_EVENTOS_KEY, { key: '', direction: null });
  ...
  useEffect(() => {
    ...
    if (vendedorSeleccionado) {
      result = result.filter((evento) => 
        evento.tarea_vendedor_id === vendedorSeleccionado
      );
    }
    ...
    if (searchTerm) {
      ...
      result = result.filter((evento) => {
        // Buscar en título
        if (evento.titulo && evento.titulo.toLowerCase().includes(searchTermLower)) {
          return true;
        }
        // Buscar en descripción
        if (evento.descripcion && evento.descripcion.toLowerCase().includes(searchTermLower)) {
          return true;
        }
        // Buscar en nombre de vendedor si está disponible
        const nombreVendedor = getNombreVendedor(evento.tarea_vendedor_id);
        if (nombreVendedor && nombreVendedor.toLowerCase().includes(searchTermLower)) {
          return true;
        }
        ...
        if (evento.tarea_client_id) {
          ...
          if (nombreCliente && nombreCliente.toLowerCase().includes(searchTermLower)) {
            return true;
          }
        }
        return false;
    ...
    if (sortConfig.key && sortConfig.direction) {
      result.sort((a, b) => {
        ...
        if (sortConfig.key === 'fecha_realizacion') {
          ...
          const dateA = a[sortConfig.key as keyof Evento] ? new Date(a[sortConfig.key as keyof Evento] as string).getTime() : 0;
          const dateB = b[sortConfig.key as keyof Evento] ? new Date(b[sortConfig.key as keyof Evento] as string).getTime() : 0;
  ...
  }, [eventos, estadosSeleccionados, vendedorSeleccionado, fechaDesde, fechaHasta, searchTerm, getNombreVendedor, getNombreCliente, sortConfig]);
            ...
            <tr>
              ...
              >
                <div className="flex items-center">
                  <span>Fecha</span> {getSortIcon('fecha_realizacion')}
                </div>
              </th>
              <th
                scope="col"
