...
export const EventoItem = ({
  ...
  return (
    <div
      ...
      className={`border rounded-lg p-4 cursor-pointer hover:shadow-md transition ${getEventoCardBgClass(
        evento.estado_tarea
      )} overflow-hidden`}
    >
          ...
          <div className="flex space-x-2 align-start">
            {evento.estado_tarea !== "Completado" && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onCambiarEstado(evento.id, "Completado");
                }}
            ...
            {evento.estado_tarea === "Completado" && (
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  onCambiarEstado(evento.id, "Pendiente");
                }}
      ...
      <div className="mt-3 flex flex-wrap justify-between text-sm">
        ...
        <span
          className={`px-2 py-1 rounded-full text-xs font-medium ml-auto ${getEstadoClass(
            evento.estado_tarea
          )}`}
        >
          {evento.estado_tarea}
